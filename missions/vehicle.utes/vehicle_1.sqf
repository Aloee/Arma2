...
private ["_hasname","_delay","_deserted","_respawns","_noend","_dead","_nodelay","_timeout","_position","_dir","_effect","_rounds","_run","_unit","_explode","_dynamic","_unitinit","_haveinit","_unitname","_type","_shape","_group"];
...
_nodelay = false;
_group = group _unit;
if(!isNull (driver _unit) && driver _unit != _unit)then{_shape = typeOf (driver _unit)};
...
{	
  ...
  if (_dead) then 
	{	
		...
		_unit setDir _dir;
		if(!isNil "_shape" && !isNil "_group")then{
			_newGroup = createGroup (side _group); _newGroup copyWaypoints _group;
			[_unit, _newGroup, false, "", _shape] call bis_fnc_spawnCrew;
		};
