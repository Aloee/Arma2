...
if(!isNil "_group")then{
	_grp_vehs_drivers = _group getVariable "grp_vehs_drvs";
		...
		for "_indw" from 0 to ((count _waypoints)-1) do{
			...
			//_wp setWaypointStatements ["true", "_way = (group this) getVariable ""Convoy_way"";_way set [0,objNull];_way = _way - [objNull];(group this) setVariable [""Convoy_way"", _way];"+_done_way]
		};
	...
	while {count _grp_vehs_drivers > 1 && !_abortWay && !isNil "_group"} do {
		_grp_vehs_drivers = _group getVariable "grp_vehs_drvs";
				...
				if(_abortWay)then{
					[_group, _checkveh] execVM "convoy\act\way_guardBrok_cnv.sqf";
				};
