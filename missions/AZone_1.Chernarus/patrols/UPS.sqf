...
_getAreaInfo = {
	if (typeName _areamarker=="String") then {
		...
		_centerpos = getPosATL _areamarker;
		_centerX = abs(_centerpos select 0);
		_centerY = abs(_centerpos select 1);
...
// create the clones
	for "_grpcnt" from 1 to _copies do {
		...
		_lead = _grp createUnit [_unittype, getPosATL _npc, [], 0, "form"];
		...
		_c=0;
		{
			_c=_c+1;
			if (_c>1) then {
				_newunit = _grp createUnit [typeof _x, getPosATL _x, [],0,"form"];
				KRON_cloneindex = KRON_cloneindex+1;
...
if (_usetrigger!="NOTRIGGER") then {
	_trgside = switch (side _npc) do { case west: {"WEST"}; case east: {"EAST"}; case resistance: {"GUER"}; case civilian: {"CIV"}; case default {"ENEMY"}};
						...
						while {_tries<20} do {
							...
							if (_iscar) then {
								...
								if (count _roadlist>0) then {
									_targetPos = getPosATL (_roadlist select 0);
									_tries=99;
								};
							} else {
								_tries=99;
