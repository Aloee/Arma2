...
private ["_xpos", "_ypos", "_vehicles", "_units", "_airs", "_objects", "_distance", "_damage",
...
[_xpos, _ypos] execvm "fx\boom\script\damage2.sqf";

_airs = nearestobjects [[_xpos, _ypos , 0], ["Air"], argt_nuclear_radius];
{
  _distance = [_xpos, _ypos, 0] distance _x;
  _dir = asin (((getPosATL _x select 1) - _ypos) / _distance);
  if ( getPosATL _x select 0 < _xpos ) then {_dir = 180 - _dir};
  _vel = velocity _x;
...
{[_x] execvm "fx\boom\script\electro_pulse.sqf"} foreach _vehicles;
  ...
  else
  {
         ...
         _x iskindof "Tank" || _x iskindof "Ship" ) then
    {
      ...
      if ( _x iskindof "Man" ) then {_speed = _speed * 2}
        else {[_x, _speed * 2] execvm "fx\boom\script\rotate.sqf"};
      _speed = _speed * argt_nuclear_blow_speed;
      _x setvelocity [(_vel select 0) + (_speed * cos _dir),
                      (_vel select 1) + (_speed * sin _dir),
                      _speed / 3];
