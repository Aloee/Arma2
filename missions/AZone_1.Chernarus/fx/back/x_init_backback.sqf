...
#define __addbp __pSetVar [#x_pbp_id, _p addAction [_s call FUNC(GreyText), "fx\back\x_backpack.sqf",[],-1,false]]
	...
	if ((_this select 0) == 0) then {
		...
		if (count __pGetVar(GVAR(player_backpack)) == 0) then {
			if (primaryWeapon _p != "" && primaryWeapon _p != " ") then {
				_s = ([primaryWeapon _p,1] call FUNC(GetDisplayName)) + " " + localize "STR_action_tobackpack";
				if (__pGetVar(GVAR(pbp_id)) == -9999) then {__addbp};
			};
		} else {
			_s = localize "STR_action_backbackpack" + " " + ([__pGetVar(GVAR(player_backpack)) select 0,1] call FUNC(GetDisplayName));
			if (__pGetVar(GVAR(pbp_id)) == -9999) then {__addbp};
		};
	};
};
...
if (GVAR(prim_weap_player) != "" && GVAR(prim_weap_player) != " ") then {
	_s = ([GVAR(prim_weap_player),1] call FUNC(GetDisplayName)) + " " + localize "STR_action_tobackpack";
	__addbp;
};
...
FUNC(bponact) = {
	...
	if (__pGetVar(GVAR(pbp_id)) == -9999 && count (__pGetVar(GVAR(player_backpack))) == 0 && GVAR(prim_weap_player) != "" && GVAR(prim_weap_player) != " ") then {
		__pSetVar [QUOTE(GVAR(pbp_id)), player addAction [format [localize "STR_action_tobackpack2", [GVAR(prim_weap_player),1] call FUNC(GetDisplayName)] call FUNC(GreyText), "fx\back\x_backpack.sqf",[],-1,false]];
	};
};
