...
if (count __pGetVar(GVAR(player_backpack)) == 0) then {
	...
	if (_primary != "") then {
		...
		for "_e" from 0 to count _mag_types do {
			_mag_types set [_e, toUpper (_mag_types select _e)];
		};
		...
		_p = player;
		_s = localize "STR_action_backbackpack" + " " + ([_primary,1] call FUNC(GetDisplayName));
		if (_pistol != "") then {
			_p addWeapon _pistol; _p selectWeapon _pistol;
		};
		if (__pGetVar(GVAR(pbp_id)) == -9999) then {__pSetVar [QUOTE(GVAR(pbp_id)), _p addAction [_s call FUNC(GreyText), "fx\back\x_backpack.sqf",[],-1,false]]};
	...
	if (_primary == "") then {
		...
		_s = format [localize "STR_action_tobackpack2", [__pGetVar(GVAR(player_backpack)) select 0,1] call FUNC(GetDisplayName)];
		...
		__pSetVar [QUOTE(GVAR(pbp_id)), _p addAction [_s call FUNC(GreyText), "fx\back\x_backpack.sqf",[],-1,false]];
	} else {
		_mag_types = getArray(configFile>> "cfgWeapons" >> _primary >> "magazines");
		for "_e" from 0 to count _mag_types do {
			_mag_types set [_e, toUpper (_mag_types select _e)];
		...
		_s = localize "STR_action_backbackpack" + " " + ([_primary,1] call FUNC(GetDisplayName));
		_id = __pGetVar(GVAR(pbp_id));
		if (_id == -9999) then {__pSetVar [QUOTE(GVAR(pbp_id)), _p addAction [_s call FUNC(GreyText), "fx\back\x_backpack.sqf",[],-1,false]]};
		if (alive player) then {GVAR(backpack_helper) = []};
	};
};