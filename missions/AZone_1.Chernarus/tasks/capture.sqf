...
if(typeName _this == "STRING")then{
	...
	switch(_this)do{
		case "Description" : {_return = "STR_tasks_captureD"};
	};
...
_return
	...
	if(local player)then{
		...
		_task = [
			...
			_taskType,
			_pos,
			localize "STRD_task_captureTitle", 
			format [localize "STRD_task_captureTitlePers", _owner getVariable "nameDisplay"],
			localize "STR_tasks_captureD"
	...
	if(isDedicated || isServer)then{
		...
		_return = "STRD_task_captureDone"; 
		
		_location = [(side (call compile _ownerName)), []]  call sfx_selectForCaptureLocation;
		if(!isNull _location)then{
			...
			_return = [_ownerName, _taskName, _taskType, _pos];
			...
			[_location, _taskName, _taskType, _ownerName] spawn {
				...
				//remove
				[_taskName, _ownerName, _taskType, [localize "STRD_task_captureTitle", "Succeeded", ""], [[], []]] call sfx_ZONA_removeTask;
			};
		};
