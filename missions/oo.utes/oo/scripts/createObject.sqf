...
if (isNil "_this") exitWith {diag_log "Log: [createObject] no parameters defined!"; objNull};
if ((typeName _this) != (typeName [])) exitWith {diag_log "Log: [createObject] list of parameters must be an Array!"; objNull};
if ((count _this) < 1) exitWith {diag_log ("Log: [createObject] " + (count _this) + " parameters provided, 1 expected!"); objNull};

private ["_className"];
_className = _this select 0;
if ((typeName _className) != (typeName "")) exitWith {diag_log "Log: [createObject] class name (0) must be a String!"; objNull};

private ["_class", "_return"];
_class = missionConfigFile >> "CfgOO" >> _className;

if ((configName _class) == "") exitWith {diag_log "Log: [createObject] class does not exist in CfgOO!"; objNull};
...
if ((configName _attributes) != "") then 
{
	...
	for "_i" from 0 to ((count _attributes) - 1) do 
	{
		...
		if ((typeName _attribName) != (typeName "")) exitWith {diag_log "Log: [createObject] attribute name should be a String!"; _return = objNull};
		if (_attribName == "") exitWith {diag_log "Log: [createObject] attribute name cannot be empty!"; _return = objNull};
		
		_attribType = getText (_attribute >> "type");
		if ((typeName _attribType) != (typeName "")) exitWith {diag_log "Log: [createObject] attribute type should be a String!"; _return = objNull};
		if (!(_attribType in ["SCALAR", "STRING", "BOOL", "ARRAY", "OBJECT", "CONFIG", "GROUP", "TASK", "SIDE"])) exitWith {diag_log "Log: [createObject] attribute type should be one of SCALAR, STRING, BOOL, ARRAY, OBJECT, CONFIG or GROUP!"; _return = objNull};
...
diag_log ("Log: [createObject] created instance of class: " + _className);
