...
while {true} do {
	...
	_statistic = [];
	{
		private ["_trigger", "_monstParams", "_amount", "_p_distances", "_zombies", "_persent_zombies", "_abandoned"];
		...
		_abandoned = true;
			...
			if(count (list _trigger) > 0)then{
				{
					...
					if({alive _x and not (_x isKindOf "DAP_ARTEFACT")} count (list _trigger) > 0)then{_abandoned = false};
			...
			//open zone
				...
				if(count _zombies == 0 and isNil{_trigger getVariable "process"} and (count FZ_ZOMBIES < FZ_MAX_ZOMBIE_COUNT))then{
					...
					[_trigger, _monstParams] execVM "core\zombies\cly_z_generator.sqf";
				};
			}else{
			//clear zone
